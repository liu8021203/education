...
public class MediaController extends FrameLayout {
  ...
  private OnClickListener fullClickListener;
  private PlayStateCallBackListener mPlayStateCallBackListener;
  ...
  public void setFullClickListener(OnClickListener fullClickListener) {
    this.fullClickListener = fullClickListener;
  }

  public void setPlayStateCallBackListener(PlayStateCallBackListener playStateCallBackListener) {
    mPlayStateCallBackListener = playStateCallBackListener;
  }
  ...
  private void initControllerView(View v) {
    ...
    ivFull = (ImageView) v.findViewById(getResources().getIdentifier("iv_full", "id", mContext.getPackageName()));
    ivFull.setOnClickListener(fullClickListener);
...
//        setWindowLayoutType();
        ...
        mWindow.showAtLocation(mAnchor, Gravity.NO_GRAVITY, anchorRect.left, mAnchor.getHeight() + UtilPixelTransfrom.dip2px(getContext(), 15) + statusBarHeight);
      }
      mShowing = true;
  ...
  private void doPauseResume() {
    if (mPlayer.isPlaying()) {
      if (mPlayStateCallBackListener != null) {
        mPlayStateCallBackListener.pause();
      }
      mPlayer.pause();
    } else {
      if (mPlayStateCallBackListener != null) {
        mPlayStateCallBackListener.start();
      }
      mPlayer.start();
    }
    updatePausePlay();
  ...
  public interface PlayStateCallBackListener{
    public void start();
    public void pause();
  }
